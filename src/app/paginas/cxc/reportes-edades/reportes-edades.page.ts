...
@Component({
  ...
  templateUrl: './reportes-edades.page.html',
  styleUrls: ['./reportes-edades.page.scss'],
})
export class ReportesEdadesPage implements OnInit {
...
ngOnInit() {  /* this.tableResults('99999', '', ''); */ }
  ...
  tableResults( idsucursal: string, fechaDesde: string , fechaHasta: string/* , tipoReporte: string  */ ) {
    this.load = true;
    ...
    this.jsonService.postJson('https://app.networkingsas.com/app/cxc/edades.php', formData).subscribe
    ((res: Array<string>) => {
      console.log(res);
      if (!res) {
        this.records = false;
