...
@Component({
  ...
  templateUrl: './ventas-dias.page.html',
  styleUrls: ['./ventas-dias.page.scss'],
})
export class VentasDiasPage implements OnInit {
...
ngOnInit() { }
  ...
  tableResults( idsucursal: string, fechaDesde: string , fechaHasta: string, idcategoria: string ) {
    ...
    this.jsonService.postJson('https://app.networkingsas.com/app/reportes/ventasDias.php', formData).subscribe
    ((res: Array<string>) => {
      console.log(res);
      if (!res) {
        this.records = false;
