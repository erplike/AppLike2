...
@Component({
  ...
  templateUrl: './pospago-segmento.page.html',
  styleUrls: ['./pospago-segmento.page.scss'],
})
export class PospagoSegmentoPage implements OnInit {
...
ngOnInit() { }
  ...
  tableResults( idsucursal: string, fechaDesde: string , fechaHasta: string, segmento: string, filtro: string ) {
    this.records = false;
    ...
    this.jsonService.postJson('https://app.networkingsas.com/app/reportes/pospagoSegmento.php', formData).subscribe
    ((res: Array<string>) => {
      console.log(res);
      if (!res) {
        this.records = false;
