...
@Component({
  ...
  templateUrl: './ventas-mes.page.html',
  styleUrls: ['./ventas-mes.page.scss'],
})
export class VentasMesPage implements OnInit {
...
ngOnInit() { }
  ...
  tableResults( idsucursal: string, fechaDesde: string , fechaHasta: string, idcategoria: string ) {
    ...
    this.jsonService.postJson('https://app.networkingsas.com/app/reportes/ventasMes.php', formData).subscribe
    ((res: Array<string>) => {
      console.log(res);
      if (!res) {
        this.records = false;
