...
@Component({
  ...
  templateUrl: './ventas-por-marca.page.html',
  styleUrls: ['./ventas-por-marca.page.scss'],
})
export class VentasPorMarcaPage implements OnInit {
...
ngOnInit() { }
  ...
  tableResults( idsucursal: string, fechaDesde: string , fechaHasta: string, idcategoria: string ) {
    ...
    this.jsonService.postJson('https://app.networkingsas.com/app/reportes/ventasMarca.php', formData).subscribe
    ((res: Array<string>) => {
      console.log(res);
      if (!res) {
        this.records = false;
