...
@Component({
  ...
  templateUrl: './compras-marca.page.html',
  styleUrls: ['./compras-marca.page.scss'],
})
export class ComprasMarcaPage implements OnInit {
...
ngOnInit() { }
  ...
  tableResults( idsucursal: string, fechaDesde: string , fechaHasta: string, idcategoria: string ) {
    ...
    this.jsonService.postJson('https://app.networkingsas.com/app/reportes/comprasMarca.php', formData).subscribe
    ((res: Array<string>) => {
      console.log(res);
      if (!res) {
        this.records = false;
