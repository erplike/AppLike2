...
@Component({
  ...
  templateUrl: './ventas-semestre.page.html',
  styleUrls: ['./ventas-semestre.page.scss'],
})
export class VentasSemestrePage implements OnInit {
...
ngOnInit() { }
  ...
  tableResults(  anio: string ,idcategoria: string, idsucursal: string, semestre: string  ) {
    ...
    this.jsonService.postJson('https://app.networkingsas.com/app/reportes/ventasSemestre.php', formData).subscribe
    ((res: Array<string>) => {
      console.log(res);
      if (!res) {
        this.records = false;
