...
@Component({
  ...
  templateUrl: './inconsistencias.page.html',
  styleUrls: ['./inconsistencias.page.scss'],
})
export class InconsistenciasPage implements OnInit {
...
ngOnInit() {  this.tableResults('99999', '', ''); }
  ...
  tableResults( filtro: string, fechaDesde: string , fechaHasta: string ) {
    this.records = false;
    const formData = new FormData();
    formData.append('filtro', filtro);
    formData.append('fechaDesde', fechaDesde);
    formData.append('fechaHasta', fechaHasta);
    this.jsonService.postJson('https://app.networkingsas.com/app/reportes/inconsistencia.php', formData).subscribe
    ((res: Array<string>) => {
      console.log(res);
